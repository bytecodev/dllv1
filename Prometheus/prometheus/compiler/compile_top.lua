...
return function(Compiler)
    function Compiler:compileTopNode(node)
        ...
        -- Some scopes (e.g. synthetic wrapper scopes for IfStatement
        -- conditions created while building the binary dispatch tree) never
        -- get __depth set directly, because __depth is only assigned to
        -- scopes that belong to an actual Block AST node (see below). Walk
        -- up the parent chain to the nearest ancestor that does have it,
        -- since such a wrapper scope always lives at the same function
        -- nesting level as its nearest tagged ancestor. Falls back to 0 if
        -- none is found (e.g. the scope chain bottoms out at global scope).
        local function resolveScopeDepth(scope)
            local s = scope;
            while s and s.__depth == nil do
                s = s.parentScope;
            end
            if s then
                return s.__depth;
        ...
        visitast(node, function(node, data)
            ...
            if varAccessLookup[node.kind] then
                if not node.scope.isGlobal then
                    if resolveScopeDepth(node.scope) < data.functionData.depth then
    ...
    function Compiler:compileFunction(node, funcDepth)
        ...
        if isVarargFunction then
            ...
            retrieveExpression = Ast.FunctionCallExpression(Ast.VariableExpression(self.scope, self.createVarargClosureVar), {
                self:blockIdExpression(block.id),
