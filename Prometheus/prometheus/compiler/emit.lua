...
return function(Compiler)
    ...
    function Compiler:emitContainerFuncBody()
        ...
        local function buildBlockThresholdCondition(scope, leftId, rightId, useAndOr)
            ...
            local boundExpr = self:thresholdExpression(bound);
        ...
        local function buildElseifChain(tb, l, r, pScope)
            ...
            if condStyle == 1 then
                -- pos < bound
                condition = Ast.LessThanExpression(self:pos(ifScope), self:thresholdExpression(bound));
                trueBlock, falseBlock = lBlock, rBlock;
            elseif condStyle == 2 then
                -- bound > pos
                condition = Ast.GreaterThanExpression(self:thresholdExpression(bound), self:pos(ifScope));
                trueBlock, falseBlock = lBlock, rBlock;
            else
                -- Equivalent split using strict > with branches reversed.
                condition = Ast.GreaterThanExpression(self:pos(ifScope), self:thresholdExpression(bound));
