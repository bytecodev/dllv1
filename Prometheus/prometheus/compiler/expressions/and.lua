...
return function(self, expression, funcDepth, numReturns)
    ...
    self:addStatement(self:setRegister(scope, self.POS_REGISTER, Ast.OrExpression(Ast.AndExpression(self:register(scope, lhsReg), self:blockIdExpression(block1.id)), self:blockIdExpression(block2.id))), {self.POS_REGISTER}, {lhsReg}, false);
    self:freeRegister(lhsReg, false);
    do
        ...
        self:addStatement(self:setRegister(scope, self.POS_REGISTER, self:blockIdExpression(block2.id)), {self.POS_REGISTER}, {}, false);
    end
