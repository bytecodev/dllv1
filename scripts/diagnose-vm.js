...
const output=fs.readFileSync(path.join(results,'04_stealanegg.medium.lua'),'utf8');
const metadata=JSON.parse(fs.readFileSync(path.join(results,'04_stealanegg.build.json'),'utf8'));
const hash=value=>createHash('sha256').update(value).digest('hex');
assert.equal(hash(output),metadata.outputSha256,'output must match its build metadata');
let source;
...
for(const count of [0,1000,5000]) {
  for(const [variant,code] of [['source',source],['medium',output]]) {
    const fixture=`
local objects={}
for i=1,${count} do
 local item={value=1}
...
`;
    ...
    fs.writeFileSync(path.join(results,'vm-diagnostic.json'),JSON.stringify({
      sourceSha256:metadata.sourceSha256,outputSha256:metadata.outputSha256,
